...
class HateSpeechDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.data_name = f"hatespeech_data_{self.preprocessing_args[1]}_{self.preprocessing_args[0]}_{self.folds_num}.csv"
        self.label_columns = (
            HateSpeechDataset.LABEL_COLUMNS if label_columns is None else label_columns
        )
    ...
    #     data.rename({"comment": "text"}, inplace=True)
    ...
    def download_data(self) -> None:
        ...
        df.rename(columns={"comment_id": "text_id"}, inplace=True)
        df.to_csv("data/train/hate_speech.csv", index=False)
    ...
    def prepare_data(self) -> None:
        ...
        data = preprocessing(
            ...
            data_file="hate_speech.csv",
            annot_file="hate_speech.csv",
    ...
    def get_split(self, split: str) -> HateSpeechDataset:
        splitted = self.data[self.data["split"] == split]
        dataset = HateSpeechDataset(splitted, self.label_columns)
        return dataset
